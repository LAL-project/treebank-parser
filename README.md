...
This branch of _treebank-parser_ uses [LAL 23.01](https://github.com/LAL-project/linear-arrangement-library/tree/23.01).
...
All the parameters that the application needs can be queried using the `--help` parameter. The output is the following:
	...
	usage: main.py [-h] -i infile -o outfile [--verbose VERBOSE] [--lal] {CoNLL-U} ...
	...
	optional arguments:
	  ...
	  --lal                 Use the debug compilation of LAL ('import lal'). The script will run
	                        more slowly, but errors will be more likely to be caught. Default:
